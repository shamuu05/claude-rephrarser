...
with st.sidebar:
    ...
    model = st.selectbox("🧠 Choose Claude Model", [
    ...
    # Temperature setting
    temperature = st.slider("🌡️ Creativity Level", min_value=0.0, max_value=1.0, value=0.7, step=0.1)
...
# Main area
st.title("📄 Transcript Rewriter (Claude AI)")
...
if api_key:
        ...
        if not input_text.strip():
            ...
            with st.spinner("Rephrasing..."):
                ...
                for i, para in enumerate(paragraphs):
                    ...
                    # Update the Anthropic client with the selected model and temperature
                    os.environ["CLAUDE_MODEL"] = model
                    os.environ["CLAUDE_TEMPERATURE"] = str(temperature)
                    
                    response = rephrase_each_paragraph([para], keywords=final_prompt, api_key=api_key)
            ...
            with col4:
                word_diff = rephrased_words - original_words
                st.metric("📊 Word Difference", word_diff, delta=f"{word_diff:+d}")
    ...
    st.info("🔐 Enter your Anthropic API key to start.")
